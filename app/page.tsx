...
const slides: Slide[] = [
  ...
  },
  {
    ...
    content: <div className="week-goals">{[
      ["01", "수업 이해", "과목의 운영 방식과 학기 프로젝트의 흐름을 이해한다.", "了解课程安排与本学期项目流程。"],
      ["02", "프로젝트 공유", "자기소개와 프로젝트Ⅰ의 현재 상태를 편하게 구두로 공유한다.", "通过口头方式轻松介绍自己并分享项目Ⅰ的当前状态。"],
      ["03", "다음 주 준비", "2주차 프로젝트 계획 공유를 위한 준비 사항을 확인한다.", "确认第2周项目计划分享的准备事项。"],
    ].map(([number, title, detail, chinese]) => <article key={number}><span>{number}</span><h3>{title}</h3><p>{detail}</p><small>{chinese}</small></article>)}</div>,
  },
  {
    ...
    content: <div className="today-layout"><div className="today-list">{[["수업 안내", "과목의 목표, 평가 방식, 주차별 진행, 과제 제출 방식을 간단히 안내합니다."], ["자기소개와 프로젝트 소개", "관심 분야와 프로젝트Ⅰ에서 진행한 내용을 자유롭게 이야기합니다."], ["2주차 프로젝트 계획 공유 안내", "다음 주에 공유할 프로젝트 계획의 형식과 준비 항목을 확인합니다."], ["질의응답", "수업과 프로젝트 진행에 관한 질문을 확인합니다."]].map(([title, detail], i) => <article key={title}><span>0{i + 1}</span><div><h3>{title}</h3><p>{detail}</p></div><Check /></article>)}</div><aside><Users /><strong>오늘은<br />발표 자료가<br />필요 없습니다.</strong><span>无需准备汇报材料</span></aside></div>,
  },
  {
    index: "07", eyebrow: "1주차 과제", chineseEyebrow: "第1周作业",
    title: "별도 제출 없이\n다음 주 공유를 준비합니다", chineseTitle: "无需单独提交，准备下周分享",
    note: "이번 주에는 별도 제출이 없습니다. 다음 주 프로젝트 계획 공유를 위해 세 가지 정보를 정리해 오도록 안내합니다.",
    content: <div className="assignment-layout"><div className="assignment-head"><ClipboardList /><div><span>1주차 별도 제출 없음 · 第1周无需单独提交</span><h3>프로젝트 계획 공유 준비</h3><p>2주차 프로젝트 계획 공유를 위해 필요한 정보를 정리합니다。</p></div></div><ol>{[
      ["프로젝트 제목과 팀 구성", "项目名称与团队成员"],
      ["프로젝트Ⅰ의 현재 상태와 어려운 점", "项目Ⅰ的当前状态与遇到的困难"],
      ["이번 학기에 구현하고 싶은 핵심 AI 기능", "本学期希望实现的核心AI功能"],
    ].map(([item, chinese], i) => <li key={item}><span>{i + 1}</span><div><strong>{item}</strong><small>{chinese}</small></div></li>)}</ol><div className="submission-box"><span>이번 주 제출 · 本周提交</span><strong>없음</strong><p>정리한 내용은 2주차 프로젝트 계획 공유에 사용합니다.</p></div></div>,
  },
  {
    index: "08", eyebrow: "마무리", chineseEyebrow: "结束",
