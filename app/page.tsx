    ...
    <div className="ai-type-detail">
      <section className="ai-type-definition">
        ...
        <figure className="ai-type-image">
          ...
          <figcaption>{imageCaption}<small>{chineseImageCaption}</small></figcaption>
        </figure>
      </section>
...
const slides: Slide[] = [
  {
    ...
    content: <div className="cover-content"><div className="cover-statement"><span>THU 12:00–15:00 · S06-602</span><strong>1주차 · 수업 안내와 프로젝트 소개</strong><p>第1周 · 课程说明与项目介绍</p></div><div className="course-card"><span>2026. 09. 03</span><strong>WEEK 01</strong><p>동아대학교 AI학과 · 东亚大学人工智能系<br />윤보라 · 尹宝罗</p></div></div>,
  },
  {
    ...
    content: <div className="course-grid"><article><span>01 · 만들 프로젝트<small>制作项目</small></span><h3>AI 서비스 또는 시스템<small>AI服务或系统</small></h3><p>프로젝트Ⅰ 결과를 고도화해 사용자 문제를 해결하는 서비스나 시스템을 완성합니다.<small>继续优化项目Ⅰ成果，完成解决用户问题的AI服务或系统。</small></p></article><article><span>02 · AI 기술 적용<small>应用AI技术</small></span><h3>데이터부터 서비스까지 연결<small>连接数据与服务</small></h3><p>데이터·모델·서비스 화면·로그를 연결해 하나의 사용 흐름으로 구성합니다.<small>连接数据、模型、服务界面与日志，构成完整的使用流程。</small></p></article><article><span>03 · 검증 방법<small>验证方法</small></span><h3>QA + UT<small>质量保证 + 用户测试</small></h3><p>팀이 기능과 AI 품질을 점검하고, UT로 사용 흐름을 검증합니다.<small>团队检查功能与AI质量，并通过用户测试验证使用流程。</small></p></article><article className="assessment"><span>04 · 학기 말 결과물<small>学期成果</small></span><h3>데모 + 검증 기록 + 재현<small>演示 + 验证记录 + 复现</small></h3><p>AI 데모와 QA·UT 결과를 제시하고, 코드·모델·문서를 재현 가능한 형태로 정리합니다.<small>展示AI演示与QA、用户测试结果，并整理可复现的代码、模型与文档。</small></p></article></div>,
  },
  {
    ...
    content: <div className="intro-layout"><section><Presentation /><span>01 · 프로젝트Ⅰ 수강자<small>修读过项目Ⅰ的学生</small></span><h3>지난 프로젝트와 발전 방향<small>上学期项目与发展方向</small></h3><ul><li>이름과 지난 학기 맡은 역할<small>姓名与上学期负责的角色</small></li><li>진행한 프로젝트와 핵심 AI 기능<small>参与的项目与核心AI功能</small></li><li>이번 학기에 더 발전시킬 부분<small>本学期希望继续发展的部分</small></li></ul></section><section><Bot /><span>02 · 처음 수강하는 학생<small>首次修读的学生</small></span><h3>AI로 개발하고 싶은 프로덕트<small>希望使用AI开发的产品</small></h3><ul><li>이름과 관심 있는 AI 분야<small>姓名与感兴趣的AI领域</small></li><li>개발하고 싶은 프로덕트나 서비스<small>希望开发的产品或服务</small></li><li>해결하고 싶은 문제와 대상 사용자<small>希望解决的问题与目标用户</small></li></ul></section></div>,
  },
  {
  ...
  },
  {
    ...
    content: <div className="ai-types-overview">{[[ScanLine, "01", "인식·분류", "识别与分类", "대상과 상태를 찾고 범주로 구분", "查找对象与状态并归类"], [Target, "02", "예측", "预测", "미래의 값·확률·위험 추정", "估计未来数值、概率与风险"], [UserRound, "03", "추천·개인화", "推荐与个性化", "사용자에게 맞는 후보 선택·정렬", "选择并排序适合用户的候选项"], [ImageIcon, "04", "생성", "生成", "새로운 글·이미지·음성·코드 제작", "生成新的文本、图像、音频与代码"], [MessageSquare, "05", "대화·지식 검색", "对话与知识检索", "문서 근거를 찾아 답변·요약", "从文档中查找依据并回答或总结"], [Bot, "06", "자동화·에이전트", "自动化与智能体", "목표를 받아 여러 단계의 작업 수행", "接收目标并执行多阶段任务"]].map(([Icon, number, name, chinese, role, chineseRole]) => { const TypeIcon = Icon as typeof ScanLine; return <article key={number as string}><TypeIcon /><span>{number as string}</span><h3>{name as string}<small>{chinese as string}</small></h3><p>{role as string}<small>{chineseRole as string}</small></p></article>; })}</div>,
  },
  {
  ...
  },
  {
    ...
    content: <AITypeDetail Icon={Target} number="02" definition="과거와 현재 데이터를 이용해 앞으로 발생할 값이나 가능성을 추정합니다." chineseDefinition="利用历史与当前数据估计未来的数值或发生概率。" image="/ai-types/maps-traffic-prediction.webp" imageAlt="혼잡한 도로의 교통 상황 사례 사진" imageCaption="Google Maps 교통 예측 사례" chineseImageCaption="Google Maps交通预测案例" sources={[{ ko: "과거 교통 패턴", zh: "历史交通模式" }, { ko: "현재 교통 상황", zh: "实时交通状况" }]} steps={[{ ko: "예측 모델", zh: "预测模型" }, { ko: "미래 속도·도착 시간", zh: "未来车速·到达时间" }]} example="Google Maps 교통·도착 시간 예측" chineseExample="Google Maps交通与到达时间预测" exampleDetail="과거 교통 패턴과 현재 상황을 결합해 미래 교통량과 도착 시간을 예측합니다." chineseExampleDetail="结合历史交通模式与当前状况，预测未来交通与到达时间。" verification="예측 오차 · 위험 사건 재현율 · 예측 시점별 성능" chineseVerification="预测误差 · 风险事件召回率 · 不同预测时间点的性能" />,
  },
  {
  ...
  },
  {
    ...
    content: <AITypeDetail Icon={Bot} number="06" definition="사용자가 목표를 주면 필요한 단계를 계획하고 허용된 도구로 여러 작업을 수행합니다." chineseDefinition="用户给出目标后，AI规划步骤并使用获准工具执行多阶段任务。" image="/ai-types/agent-builder-workflow.webp" imageAlt="검색과 요약 단계를 연결한 에이전트 워크플로 화면" imageCaption="Agent Builder 워크플로" chineseImageCaption="Agent Builder工作流" sources={[{ ko: "사용자 목표", zh: "用户目标" }]} steps={[{ ko: "단계 계획·도구 선택", zh: "规划步骤·选择工具" }, { ko: "문서·데이터 작업", zh: "处理文档与数据" }, { ko: "결과 확인", zh: "检查结果", decision: true }, { ko: "결과 제출·사용자 승인", zh: "提交结果·用户确认" }]} loopLabel={{ ko: "수정 필요 → 계획으로 돌아감", zh: "需要修改 → 返回计划" }} example="반복 업무를 처리하는 Workspace Agent" chineseExample="处理重复工作的Workspace Agent" exampleDetail="관련 문서와 데이터를 확인해 정해진 형식의 보고서와 공유 메시지 초안을 만듭니다." chineseExampleDetail="读取相关文档与数据，按指定格式生成报告与共享消息草稿。" verification="전체 과업 성공률 · 도구 선택 정확도 · 승인 절차 위반률" chineseVerification="完整任务成功率 · 工具选择准确率 · 审批流程违规率" />,
  },
  {
    ...
    content: <div className="case-overview case-contain"><figure><img src="/cases/notebooklm.png" alt="NotebookLM 공식 소개 이미지" /><figcaption>Google 공식 소개 이미지 · Google官方介绍图片</figcaption></figure><div className="feature-list"><article><FileStack /><div><span>분석할 자료를 넣습니다<small>添加要分析的资料</small></span><p>PDF, 문서, 웹 링크를 업로드합니다.<small>上传PDF、文档或网页链接。</small></p></div></article><article><MessageSquare /><div><span>자료에 관해 질문합니다<small>围绕资料提问</small></span><p>AI가 업로드한 자료에서 관련 내용을 찾아 답합니다.<small>AI从上传的资料中查找相关内容并回答。</small></p></div></article><article><ClipboardList /><div><span>답변의 근거를 확인합니다<small>核对回答依据</small></span><p>출처를 눌러 답변이 나온 원문 위치를 확인합니다.<small>点击出处，查看答案所依据的原文位置。</small></p></div></article><a href="https://blog.google/innovation-and-ai/products/developing-notebooklm/" target="_blank" rel="noreferrer">Google 공식 소개 · Google官方介绍 ↗</a></div></div>,
  },
  {
  ...
  },
  {
    ...
    content: <div className="feature-gallery feature-gallery-three phone-gallery"><figure><img src="/cases/master-camera.png" alt="Master Gesture 수어 인식 화면" /><figcaption><strong>수어 인식<small>手语识别</small></strong><span>손동작을 인식해 문장으로 표시<small>识别手部动作并显示为句子</small></span></figcaption></figure><figure><img src="/cases/master-text.png" alt="Master Gesture 텍스트를 수어로 바꾸는 화면" /><figcaption><strong>텍스트 → 수어<small>文字 → 手语</small></strong><span>입력한 단어를 손모양으로 표시<small>把输入的单词显示为手语动作</small></span></figcaption></figure><figure><img src="/cases/master-signbook.png" alt="Master Gesture 수어 사전 화면" /><figcaption><strong>수어 사전<small>手语词典</small></strong><span>알파벳별 손모양을 탐색<small>按字母查找手语动作</small></span></figcaption></figure></div>,
  },
  {
    ...
    content: <div className="case-overview case-contain"><figure><img src="/cases/from-your-eyes.webp" alt="FROM YOUR EYES 앱 화면" /><figcaption>서비스 화면 · 服务界面</figcaption></figure><div className="feature-list"><article><Target /><div><span>만든 이유<small>开发目的</small></span><p>시각장애인이 사진·영상 속 정보에 스스로 접근하도록 돕습니다.<small>帮助视障用户自主获取照片和视频中的信息。</small></p></div></article><article><ImageIcon /><div><span>사진·영상 설명<small>照片与视频说明</small></span><p>화면 속 대상과 상황을 문장으로 설명합니다.<small>用文字说明画面中的对象与情境。</small></p></div></article><article><Code2 /><div><span>앱과 API<small>应用与API</small></span><p>다른 서비스에서도 설명 기능을 사용할 수 있습니다.<small>其他服务也可以接入视觉说明功能。</small></p></div></article><a href="https://news.microsoft.com/source/latam/noticias-de-microsoft/anunciamos-al-campeon-mundial-de-imagine-cup-2024/" target="_blank" rel="noreferrer">Microsoft 공식 소개 · Microsoft官方介绍 ↗</a></div></div>,
  },
  {
    ...
    content: <div className="impact-layout"><div className="impact-images"><figure><img src="/cases/from-your-eyes-team.png" alt="2024 Imagine Cup 우승팀 FROM YOUR EYES" /><figcaption>2024 World Champion · 2024世界冠军</figcaption></figure><figure><img src="/cases/from-your-eyes-award.png" alt="2024 Imagine Cup World Championship 트로피" /><figcaption>Microsoft Imagine Cup</figcaption></figure></div><div className="impact-points"><article><Glasses /><div><strong>스마트 기기 연동<small>连接智能设备</small></strong><p>글래스와 워치에서도 주변 상황 설명을 들을 수 있습니다.<small>也可通过智能眼镜和手表获取环境说明。</small></p></div></article><article><Code2 /><div><strong>외부 서비스용 API<small>面向外部服务的API</small></strong><p>다른 앱에도 시각 설명 기능을 연결합니다.<small>把视觉说明功能接入其他应用。</small></p></div></article><div className="impact-result"><span>프로젝트 결과 · 项目成果</span><strong>2024 Imagine Cup 세계 우승<small>2024 Imagine Cup 世界冠军</small></strong></div></div></div>,
  },
  {
  ...
  },
  {
    ...
    content: <div className="week-plan">{[["01", "OT·팀 출발점 확인", "课程说明·确认团队起点", "팀 구성·개인 역할 정리", "整理团队与个人角色"], ["02", "프로젝트 현황 발표", "汇报项目现状", "현황 1쪽 + 저장소·데모 링크", "现状1页 + 代码库·演示链接"], ["03", "문제 근거·가설·검증 설계", "问题依据·假设·验证设计", "검증 계획서 1차본", "验证计划书初稿"], ["04", "기준선 측정·MVP 확정", "测量基线·确定MVP", "기준선 결과표 + 기능 명세", "基线结果表 + 功能说明"], ["05", "AI 기능 구현 1", "实现AI功能 1", "동작 화면 + 코드 + 이슈 목록", "运行画面 + 代码 + 问题清单"]].map(([week, activity, chineseActivity, task, chineseTask]) => <article key={week}><span>{week}주<small>第{Number(week)}周</small></span><div><strong>{activity}</strong><small>{chineseActivity}</small></div><div><b>주차 과제<small>每周作业</small></b><p>{task}<small>{chineseTask}</small></p></div></article>)}</div>,
  },
  {
    ...
    content: <div className="week-plan">{[["06", "AI 기능·화면 통합", "整合AI功能与界面", "전체 흐름 영상 + 실행 방법", "完整流程视频 + 运行方法"], ["07", "중간 점검·QA 설계", "期中检查·QA设计", "테스트 케이스 + 버그 목록", "测试用例 + 缺陷清单"], ["08", "중간 발표·데모", "期中汇报·演示", "발표 자료 + 피드백·수정 계획", "汇报资料 + 反馈·修改计划"], ["09", "기능 QA", "功能QA", "QA 기록 1차본", "QA记录初稿"], ["10", "AI 품질·성능 QA", "AI质量·性能QA", "평가 데이터 + 측정 결과표", "评估数据 + 测量结果表"]].map(([week, activity, chineseActivity, task, chineseTask]) => <article key={week}><span>{week}주<small>第{Number(week)}周</small></span><div><strong>{activity}</strong><small>{chineseActivity}</small></div><div><b>주차 과제<small>每周作业</small></b><p>{task}<small>{chineseTask}</small></p></div></article>)}</div>,
  },
  {
    ...
    content: <div className="week-plan">{[["11", "UT 설계·준비", "用户测试设计·准备", "계획서 + 대본 + 과업지", "计划书 + 主持稿 + 任务单"], ["12", "UT 1차", "用户测试第1轮", "익명 원자료 + 관찰 기록", "匿名原始数据 + 观察记录"], ["13", "추가 UT·결과 분석", "追加测试·结果分析", "결과표 + 개선 우선순위 + 한계", "结果表 + 改进优先级 + 局限"], ["14", "수정·최종 통합", "修改·最终整合", "수정 전후 비교 + 보고서 초안", "修改前后对比 + 报告初稿"], ["15", "최종 발표·시연", "期末汇报·演示", "서비스·문서·검증 결과", "服务·文档·验证结果"]].map(([week, activity, chineseActivity, task, chineseTask]) => <article key={week}><span>{week}주<small>第{Number(week)}周</small></span><div><strong>{activity}</strong><small>{chineseActivity}</small></div><div><b>주차 과제<small>每周作业</small></b><p>{task}<small>{chineseTask}</small></p></div></article>)}</div>,
  },
  {
  ...
  return (
    <main className="audience-shell" aria-label="학생용 슬라이드쇼 화면">
      ...
      <nav className="audience-controls" aria-label="슬라이드 이동">
        ...
        <Button onClick={() => go(current + 1)} disabled={current === slides.length - 1} aria-label="다음 슬라이드">다음 · 下一张 <ArrowRight /></Button>
      </nav>
    </main>
  );
}
